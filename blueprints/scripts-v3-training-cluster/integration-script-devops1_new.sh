...
blueprintfile=~/scripts/blueprint/devops1_new.json
hdprepofile=~/scripts/blueprint/hdprepo.json
hdputilfile=~/scripts/blueprint/hdputil.json
hostmappingfile=~/scripts/blueprint/hostmapping-devops1_new.json
clustername=XXXX
...
ssh -o StrictHostKeyChecking=no $host 'wget https://s3.amazonaws.com/XXX/hadoop/repo/ambari.repo -O /etc/yum.repos.d/ambari.repo'
ssh -o StrictHostKeyChecking=no $host 'wget https://s3.amazonaws.com/XXX/hadoop/repo/HDP-2.6.1.49-2.repo -O /etc/yum.repos.d/HDP-2.6.1.49-2.repo'
ssh -o StrictHostKeyChecking=no $host 'wget https://s3.amazonaws.com/XXXX/hadoop/repo/HDP-UTILS.repo -O /etc/yum.repos.d/HDP-UTILS.repo'
ssh -o StrictHostKeyChecking=no $host 'wget https://s3.amazonaws.com/XXX/hadoop/repo/HDP.repo -O /etc/yum.repos.d/HDP.repo'
...
pdsh 'cd /tmp;wget http://XXXXX.s3.amazonaws.com/AllNodes/jdk-8u151-linux-x64.rpm'
...
psql -h ${databasehost} -U XXX -d XXXX -f $dbcmdsfile
export PGPASSWORD=XXXXX
...
psql -h ${databasehost} -U XXXX -d postgres -f $dropdbcmdsfile
...
#echo "curl -ivk -H "X-Requested-By: ambari" -u admin:admin -X POST -d @${payloadfile} http://${ambariserverip}:8080/api/v1/clusters/${clustername}/request_schedules"
rm -rf $tfile
curl  -H "X-Requested-By: ambari" -u admin:admin -X POST -d @${payloadfile} http://${ambariserverip}:8080/api/v1/clusters/${clustername}/request_schedules >> $tfile 2>&1
reqid=`grep -i -w "id" ${tfile} | cut -d ':' -f2 | tr -d ' '`
sleep 1
rm -rf $tfile
curl  -H "X-Requested-By: ambari" -u admin:admin -X GET  http://${ambariserverip}:8080/api/v1/clusters/${clustername}/request_schedules/${reqid} >> $tfile 2>&1
