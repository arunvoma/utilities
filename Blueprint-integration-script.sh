...
dbcmdsfile=~/scripts/pre-database-req.sql
dbcmds1file=~/scripts/pre-database-req-1.sql
dropdbcmdsfile=~/scripts/drop-db-cmds.sql
...
ssh -o StrictHostKeyChecking=no $host 'wget https://s3.amazonaws.com/XXXX/hadoop/repo/ambari.repo -O /etc/yum.repos.d/ambari.repo'
...
ssh -o StrictHostKeyChecking=no $host 'wget https://s3.amazonaws.com/XXXXXXXX/hadoop/repo/HDP-UTILS.repo -O /etc/yum.repos.d/HDP-UTILS.repo'
ssh -o StrictHostKeyChecking=no $host 'wget https://s3.amazonaws.com/XX/hadoop/repo/HDP.repo -O /etc/yum.repos.d/HDP.repo'
...
pdsh 'cd /tmp;wget http://XXXXXXXXX.s3.amazonaws.com/AllNodes/jdk-8u151-linux-x64.rpm'
...
psql -h ${databasehost} -U XXX -d postgres -f $dbcmdsfile
export PGPASSWORD=XXX
psql -h ${databasehost} -U XXX -d ambari -f $dbcmds1file
psql -h ${databasehost} -U XXX -d ambari -f /var/lib/ambari-server/resources/Ambari-DDL-Postgres-CREATE.sql
